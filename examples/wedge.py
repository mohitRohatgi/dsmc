...
def main():
    ...
    ref_point = (0.0, 1.0)
    surf_temp = 2800.0 * 5.0 / 3.0
    # creating surface
    surf_group = SurfaceGroup()
    surf_group.add_new_group(wedge, ref_point, surf_temp)
    
    inlet = [((0.0, 0.5), (0.5, 0.5))]
    outlet = [((0.5, 1.0), (0.1, 1.0))]
    zero_grad  = [((0.0, 0.5), (0.0, 0.6268)),
		  ((0.5, 0.5), (0.5, 1.0))]
    center = (0.25, 0.75)
    length = 0.5
    width = 0.5
...
#    cell_x, cell_y = np.ceil(length / dl), np.ceil(width / dl)
    ...
    f = open('wedge_super_temperature_stag.txt', 'w')
    np.savetxt(f, temperature)
    f.close()
    
    f = open('wedge_super_number_density_0_stag.txt', 'w')
    np.savetxt(f, number_density_0)
    f.close()
    
    f = open('wedge_super_number_density_1_stag.txt', 'w')
    np.savetxt(f, number_density_1)
    f.close()
    
    f = open('wedge_super_speed_stag.txt', 'w')
    np.savetxt(f, speed)
    f.close()
    
    f = open('wedge_super_u_stag.txt', 'w')
    np.savetxt(f, u)
    f.close()
    
    f = open('wedge_super_v_stag.txt', 'w')
    np.savetxt(f, v)
    f.close()
    
    f = open('wedge_super_w_stag.txt', 'w')
    np.savetxt(f, w)
    f.close()
    
    f = open('wedge_super_pressure_stag.txt', 'w')
